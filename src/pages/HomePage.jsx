...
const HomePage = () => {
    return (
        <>
            ...
            <main>
                {/* Hero Section */}
                <section className="relative w-full min-h-[400px] sm:min-h-[500px] md:h-[600px] flex items-center justify-center px-3 sm:px-4 overflow-hidden">
                    ...
                    <div className="relative z-10 w-full max-w-4xl text-center flex flex-col items-center gap-4 sm:gap-6 md:gap-8">
                        <div className="space-y-2 sm:space-y-4">
                            <h1 className="text-white text-2xl sm:text-4xl md:text-6xl font-black leading-tight tracking-tight drop-shadow-lg">
                                Explore Sri Lanka: <br className="hidden sm:block" />
                                <span className="text-primary">Beaches, Mountains and Culture</span>
                            </h1>
                            <p className="text-white/90 text-base sm:text-lg md:text-xl font-medium max-w-2xl mx-auto drop-shadow-md">
                                Experience the magic of the teardrop island, from golden coasts to misty highlands.
                            </p>
                        </div>
                        {/* Search Bar */}
                        <div className="w-full max-w-2xl bg-white/95 dark:bg-background-dark/95 backdrop-blur rounded-lg sm:rounded-xl p-2 sm:p-3 shadow-2xl flex flex-col md:flex-row gap-2">
                            <div className="flex-1 flex items-center px-2 sm:px-4 gap-2 sm:gap-3 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700">
                                <span className="material-symbols-outlined text-slate-400 text-lg sm:text-xl">search</span>
                                <input className="w-full py-2.5 sm:py-3 bg-transparent border-none focus:ring-0 text-xs sm:text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-500" placeholder="Where do you want to go?" type="text" />
                            </div>
                            <div className="flex-1 flex items-center px-2 sm:px-4 gap-2 sm:gap-3">
                                <span className="material-symbols-outlined text-slate-400 text-lg sm:text-xl">calendar_month</span>
                                <input className="w-full py-2.5 sm:py-3 bg-transparent border-none focus:ring-0 text-xs sm:text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-500" placeholder="When are you visiting?" type="text" />
                            </div>
                            <Link to="/destinations" className="bg-primary hover:bg-primary/90 text-slate-900 font-bold py-2.5 sm:py-3 px-4 sm:px-8 rounded-lg transition-colors flex items-center justify-center gap-2 text-xs sm:text-sm whitespace-nowrap">
                                Search
                            </Link>
                        </div>
                    </div>
                ...
                <section className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-12 sm:py-16 md:py-20">
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-12 gap-3 sm:gap-4">
                        <div className="space-y-1 sm:space-y-2">
                            <span className="text-primary font-bold tracking-widest uppercase text-xs sm:text-sm">Discover</span>
                            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold">Popular Destinations</h2>
                        </div>
                        <Link to="/destinations" className="flex items-center gap-2 text-primary font-bold hover:underline text-sm sm:text-base">
                            View all
                ...
                {/* CTA Section */}
                <section className="bg-primary/5 dark:bg-primary/10 py-12 sm:py-16 md:py-20">
                    <div className="max-w-4xl mx-auto px-3 sm:px-4 text-center space-y-6 sm:space-y-8">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
                            <Link to="/trip-planner" className="w-full flex items-center justify-center sm:w-auto min-h-12 sm:min-h-14 rounded-lg sm:rounded-xl bg-primary text-slate-900 font-bold text-sm sm:text-base md:text-lg hover:shadow-lg hover:shadow-primary/20 transition-all px-4 sm:px-6">
                                Start Planning
                            </Link>
                            <Link to="/map" className="w-full flex items-center justify-center sm:w-auto min-h-12 sm:min-h-14 rounded-lg sm:rounded-xl border-2 border-primary text-primary font-bold text-sm sm:text-base md:text-lg hover:bg-primary/5 transition-all px-4 sm:px-6">
                                View Map
                            </Link>
                        </div>
                    </div>
                ...
                <section className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-12 sm:py-16">
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-4 sm:mb-6 gap-2 sm:gap-3">
                        <div>
                            <span className="text-primary font-bold tracking-widest uppercase text-xs sm:text-sm">Explore</span>
                            <h2 className="text-2xl sm:text-3xl font-bold">Interactive Map</h2>
                            <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-1">Sri Lanka's top destinations</p>
                        </div>
                        <Link to="/map" className="flex items-center gap-2 text-primary font-bold hover:underline shrink-0 text-sm sm:text-base">
                            Open Full Map <span className="material-symbols-outlined text-lg">open_in_new</span>
                        </Link>
                    </div>
                    <div className="rounded-lg sm:rounded-2xl overflow-hidden h-[250px] sm:h-[350px] md:h-[450px] relative border border-slate-200 dark:border-slate-800 shadow-xl">
                        <HomeMap />
                        {/* Overlay badges */}
                        <div className="absolute top-2 sm:top-4 left-2 sm:left-4 flex flex-col gap-1.5 sm:gap-2 pointer-events-none">
                            {SRI_LANKA_PINS.slice(0, 4).map(pin => (
                                <div key={pin.name} className="hidden sm:flex items-center gap-2 bg-white/90 dark:bg-slate-900/90 backdrop-blur px-2 sm:px-3 py-1 sm:py-1.5 rounded-full shadow text-xs font-bold border border-primary/10">
                                    <span className="material-symbols-outlined text-primary" style={{ fontSize: 12 }}>location_on</span>
                                    {pin.name}
                                </div>
                            ))}
                            <div className="flex items-center gap-2 bg-primary/90 text-white backdrop-blur px-2 sm:px-3 py-1 sm:py-1.5 rounded-full shadow text-xs sm:text-xs font-bold">
                                +{SRI_LANKA_PINS.length - 4} more
                            </div>
                        </div>
                        {/* CTA overlay button */}
                        <Link
                            to="/map"
                            className="absolute bottom-2 sm:bottom-4 right-2 sm:right-4 flex items-center gap-2 bg-primary text-slate-900 font-bold px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl shadow-xl hover:scale-105 transition-transform text-xs sm:text-sm"
                        >
